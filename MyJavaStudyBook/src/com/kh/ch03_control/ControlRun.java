...
//		System.out.println(fruit + "의 가격은 " + price + "원 입니다.");
		...
		// ---------------------------------------------------------------------------------
		/*
		 * * 반복문 : 어떤 코드를 반복적으로 실행 - 종류 [1] for 문 [2] while 문
		 */

		/*
		 * for(초기식; 조건식; 증감식) { 반복적으로 실행할 코드 작성 }
		 * 
		 * - 초기식 : 반복문이 수행될 떄 "처음 단 한번만 실행" => 보통 반복문 안에서 사용될 변수를 선언 및 초기화 - 조건식 :
		 * "반복문이 수행될 조건" => 조건식의 결과가 true일 때 코드 실행 => 조건식의 결과가 false일 떄 반복문을 종료 => 보통
		 * 초기식에 선언된 변수를 가지고 조건식을 작성 - 증감식 : 반복문을 제어하는 변수의 값을 증가시키거나 감소시키는 부분 => 보통 초기식에
		 * 선언된 변수를 가지고 증감연산자(++, --)를 사용
		 * 
		 */
		...
		/*
		 * 위의 랜덤값을 기준으로 아래와 같이 출력해보기 (ex dan == 4) 1*** *2** **3* ***4
		 */
		...
		/*
		 * do { // 무조건 1회 실행한 후 조건식의 결과를 확인하여 반복 수행 반복 실행할 코드 } while(조건식);
		 */
		int num2 = 1;
		do {
		...
		// ----------------------------------------------------------------
		/*
		 * 분기문 - continue; : 코드를 실행하다가 이 분기문을 만나면 다음 분기로 넘김 - break; : 반복문을 강제로 빠져나감(종료)
		 * * switch문 안의 break; switch문 만을 빠져나감!
		 */
		// 1 ~ 10 사이의 홀수만 출력
		for (int k = 1; k < 11; k++) {
			if (k % 2 == 0) {
		...
		while (true) {
			int landmark = (int) (Math.random() * 100 + 1);
			if(landmark % 3 == 0) {
				break;
