...
//		}
		 ...
		 * 		반복문의 조건식 부분이 i <= 배열길이 로 되어 있어 마지막 인덱스보다 큰 값까지 접근하게 된다. 
		 * 		=> 조건식 부분을 i < 배열길이 로 수정해줘야 함
		 * 2) 객체 배열의 각 인덱스 위치에 담긴 값이 null인 상태인데,
		 * 		이 상태에서 객체 내의 메소드로 접근하려함 (NullPointerException 예외발생!)
		 * 		=> 메소드 접근 전에 각 인덱스 위치에 객체를 생성해줘야 한다.
		 */
		for(int i = 0; i < products.length; i++) {
